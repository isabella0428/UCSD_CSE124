...
void Client::receive(int socket_id, std::string path)
{
    ...
    std::string received(&fullResponse[0], totalLength);

    // Find filename
    int idx = received.find("Filename: ");
    if (idx == (int)std::string::npos)
    {
        return;
    }
    int fileContentId = received.find("\r\n\r\n");
    if (fileContentId == (int)std::string::npos)
    {
        return;
    }
    std::string response = received.substr(0, fileContentId + 4);
    std::cout << response;
    std::string filename = received.substr(idx + 10, received.find("Last") - idx - 12);
    std::string file = received.substr(fileContentId + 4, response.size() - fileContentId - 4);
